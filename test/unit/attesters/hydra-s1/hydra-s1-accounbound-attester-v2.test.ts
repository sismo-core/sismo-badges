...
describe('Test HydraS1 Accountbound Attester V2 contract', () => {
  ...
  describe('Generate Attestations', () => {
    ...
    it('Should generate a proof with Hydra S1 Prover and verify it onchain using the attester', async () => {
      ...
      expect(
        await hydraS1AccountboundAttesterv2.getDestinationOfTicket(
          BigNumber.from(inputs.publicInputs.userTicket)
        )
      ).to.equal(request.destination);

      expect(
        await hydraS1AccountboundAttesterv2.getTicketCooldownStart(
          BigNumber.from(inputs.publicInputs.userTicket)
        )
      ).to.be.eql(0);

      expect(
        await hydraS1AccountboundAttesterv2.getTicketBurnCount(
          BigNumber.from(inputs.publicInputs.userTicket)
        )
      ).to.be.eql(0);
    ...
    it('Should be able to change the destination, deleting the old attestation', async () => {
      ...
      await expect(generateAttestationsTransaction)
        .to.emit(hydraS1AccountboundAttesterv2, 'TicketDestinationUpdated')
        .withArgs(inputs.publicInputs.userTicket, destination2.identifier);
      await expect(generateAttestationsTransaction)
        .to.emit(hydraS1AccountboundAttesterv2, 'TicketSetOnCooldown')
    ...
    it('Should revert if the ticket is in cooldown', async () => {
      const burnCount = await hydraS1AccountboundAttesterv2.getTicketBurnCount(
        BigNumber.from(inputs.publicInputs.userTicket)
      );

      const destination = await hydraS1AccountboundAttesterv2.getDestinationOfTicket(
        BigNumber.from(inputs.publicInputs.userTicket)
      );
      ...
      await expect(
        ...
        `TicketOnCooldown(${
          inputs.publicInputs.userTicket
    ...
    it('Should renew the timestamp even if the ticket is in cooldown', async () => {
      const evmSnapshotId = await evmSnapshot(hre);
      const latestCooldownStart = await hydraS1AccountboundAttesterv2.getTicketCooldownStart(
        BigNumber.from(inputs.publicInputs.userTicket)
      );
